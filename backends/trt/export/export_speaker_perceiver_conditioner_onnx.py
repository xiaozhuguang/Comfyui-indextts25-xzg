"""Export speaker perceiver conditioner to ONNX.

Usage:
    python export_speaker_perceiver_conditioner_onnx.py [--output speaker_perceiver_conditioner.onnx]
"""
...
class SpeakerPerceiverConditioner(nn.Module):
    """Wraps ConformerEncoder + PerceiverResampler for ONNX export."""

    def __init__(self, gpt_model):
        super().__init__()
        self.conditioning_encoder = gpt_model.conditioning_encoder
        self.perceiver_encoder = gpt_model.perceiver_encoder
        self.cond_mask_pad = gpt_model.cond_mask_pad
    ...
    def forward(self, speech_input: torch.Tensor, lengths: torch.Tensor) -> torch.Tensor:
        ...
        Returns:
            conds: (B, 32, 1280) conditioning latents
        """
        encoder_out, mask = self.conditioning_encoder(speech_input, lengths)
        conds_mask = self.cond_mask_pad(mask.squeeze(1))
        conds = self.perceiver_encoder(encoder_out, conds_mask)
        return conds
...
def export_onnx(output_path, device="cuda:0"):
    ...
    wrapper = SpeakerPerceiverConditioner(gpt).eval()
    del gpt
...
def main():
    ...
    parser.add_argument("--output", default="speaker_perceiver_conditioner.onnx")
