"""Build a TensorRT engine from the latent projector ONNX model.

Usage:
    python build_latent_projector_trt.py [--onnx latent_projector.onnx] [--output latent_projector.engine]
"""
...
def build_engine(onnx_path, engine_path, fp16=True,
    ...
    profile.set_shape("latent",
                       min=(min_batch, min_T, 1280),
                       opt=(opt_batch, opt_T, 1280),
                       max=(max_batch, max_T, 1280))
    # codes: (B, T)
    profile.set_shape("codes",
                       min=(min_batch, min_T),
                       opt=(opt_batch, opt_T),
                       max=(max_batch, max_T))
...
def main():
    parser = argparse.ArgumentParser(description="Build TRT engine from latent projector ONNX")
    parser.add_argument("--onnx", default="latent_projector.onnx")
    parser.add_argument("--output", default="latent_projector.engine")
