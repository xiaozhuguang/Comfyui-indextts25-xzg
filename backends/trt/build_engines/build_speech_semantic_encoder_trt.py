"""Build a TensorRT engine from the speech semantic encoder ONNX model.

Usage:
    python build_speech_semantic_encoder_trt.py [--onnx speech_semantic_encoder.onnx] [--output speech_semantic_encoder.engine]
"""
...
def build_engine(onnx_path, engine_path, fp16=True,
    ...
    profile.set_shape("input_features",
                       min=(min_batch, min_T, 160),
                       opt=(opt_batch, opt_T, 160),
                       max=(max_batch, max_T, 160))
    # attention_mask: (B, T)
...
def main():
    parser = argparse.ArgumentParser(description="Build TRT engine from speech semantic encoder ONNX")
    parser.add_argument("--onnx", default="speech_semantic_encoder.onnx")
    parser.add_argument("--output", default="speech_semantic_encoder.engine")
