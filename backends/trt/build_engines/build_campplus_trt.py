"""Build a TensorRT engine from the CAMPPlus speaker style ONNX model.

Usage:
    python build_campplus_trt.py [--onnx campplus.onnx] [--output campplus.engine]
"""
...
def build_engine(onnx_path, engine_path, fp16=True,
    ...
    # x: (B, T, 80)
    profile.set_shape("x",
                       min=(min_batch, min_T, 80),
                       opt=(opt_batch, opt_T, 80),
                       max=(max_batch, max_T, 80))
...
def main():
    parser = argparse.ArgumentParser(description="Build TRT engine from CAMPPlus ONNX")
    parser.add_argument("--onnx", default="campplus.onnx")
    parser.add_argument("--output", default="campplus.engine")
    parser.add_argument("--fp16", action="store_true", default=True)
    parser.add_argument("--no-fp16", dest="fp16", action="store_false")
    parser.add_argument("--min-T", type=int, default=50)
