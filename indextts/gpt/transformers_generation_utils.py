...
# Internal transformers helpers removed in newer versions. IndexTTS never uses
# assisted/speculative decoding, so keep these optional and fail only if reached.
try:
    from transformers.generation.candidate_generator import (
    ...
    def _crop_past_key_values(*_args, **_kwargs):
        raise RuntimeError("Assisted/speculative decoding is not supported by this transformers version.")

    def _prepare_attention_mask(*_args, **_kwargs):
        raise RuntimeError("Assisted/speculative decoding is not supported by this transformers version.")

    def _prepare_token_type_ids(*_args, **_kwargs):
        raise RuntimeError("Assisted/speculative decoding is not supported by this transformers version.")
...
# Present in older transformers, moved/renamed in newer ones. Fall back to a
# minimal mapping so the module loads across versions.
try:
    from transformers.generation.configuration_utils import NEED_SETUP_CACHE_CLASSES_MAPPING
except ImportError:  # pragma: no cover
    NEED_SETUP_CACHE_CLASSES_MAPPING = {"static": StaticCache, "offloaded": OffloadedCache}

# Only used when the user requests a quantized KV cache (not used by IndexTTS).
try:
        ...
        if generation_config.cache_implementation is not None:
            if generation_config.cache_implementation in NEED_SETUP_CACHE_CLASSES_MAPPING:
                ...
                if QuantizedCacheConfig is None:
                    raise ValueError(
                        "The installed transformers version does not support the quantized KV cache."
                    )
                cache_config = (
